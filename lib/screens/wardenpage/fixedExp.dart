...
class FixedExp extends StatefulWidget {
  final String? uid;

  const FixedExp({Key? key, this.uid}) : super(key: key);

  @override
  _MonthlyExpState createState() => _MonthlyExpState();
}

class _MonthlyExpState extends State<FixedExp> {
  ...
  Future<String> getName(String uid) async {
    ...
    if (resp.data.isNotEmpty) {
      ...
      var response = await supabase
          ...
          print("resp"+response.data.toString()+uid);

      if (response.error == null && response.data.isNotEmpty) {
        return "${response.data[0]['first_name']} ${response.data[0]['last_name']}";
      } else {
        // Handle the case when user is not found in either 'users' or 'deleted_user' table
  ...
  Future<void> addExpense() async {
    ...
    if (date.isNotEmpty && amount > 0) {
      ...
      final response = await supabase.from('fixed').insert({
        'date': date,
        'amount': amount,
        'remark': remark,
        'u_id': widget.uid,
            ...
            child: ValueListenableBuilder<List<ExpenseItem>>(
              ...
              builder: (context, expenses, _) {
                return ShowList(
                  ...
                  uid: widget.uid!,
                  fetchDate: () => setState(() {
                    fetchExpenses(selectedDate!.month.toString(), selectedDate!.year);
                  }),
                );
              },
            ),
